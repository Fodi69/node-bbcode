...
exports.parse = function(post, cb) {
  ...
  var tagname_re = /^\/?(?:b|i|u|pre|center|samp|code|colou?r|size|noparse|url|link|s|q|(block)?quote|li)$/i;
      ...
      switch (m2l) {
        ...
        case "size":
          if(!m3 || !number_re.test(m3))
            m3 = "1";
          opentags.push(new taginfo_t(m2l, "</span>"));
          return "<span style=\"font-size: " + Math.min(Math.max(m3, 70), 200) / 100 + "em\">";
